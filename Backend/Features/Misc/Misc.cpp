...
{
	CBaseEntity* pLocal = c_globals::LocalPlayer;
	Ray_t ray;
	ray.Init( start, end );
...
{
	auto pLocal = c_globals::LocalPlayer;
...
{
	Vector Velocity = c_globals::LocalPlayer->GetVelocity();
	...
	VelLeft.Normalize();
	VelRight.Normalize();
	float frac = GetTraceFractionWorldProps( c_globals::LocalPlayer->GetAbsOrigin(), c_globals::LocalPlayer->GetVelocity() * TICKS_TO_TIME( 5 ) );
	Vector origin = Vector( 0, 0, 5 ) + c_globals::LocalPlayer->GetAbsOrigin() + c_globals::LocalPlayer->GetVelocity().GetNormalized() * frac;
	Velocity.z = 0;
	float Speed = Velocity.Length();
