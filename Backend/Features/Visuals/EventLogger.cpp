...
void CEventLogger::Draw()
{
	...
	for ( auto &l : logs )
	{
		auto addy = ( c_config::get()->b["misc_showprefix"] ? Drawing->TextSize( "[gideonproject] ", Drawing->fonts.esp_main ).right : 0 );

		if ( c_config::get()->b["misc_showprefix"] )
			Drawing->Text( 5, 5 + offset + l.yOffset, "[gideonproject] ", Drawing->fonts.esp_main, c_config::get()->imcolor_to_ccolor(c_config::get()->c["misc_prefixcolor"]));

		Drawing->Text( 5 + addy, 5 + offset + l.yOffset, l.text.c_str(), Drawing->fonts.esp_main, CColor( 255, 255, 255, l.alpha ) );
