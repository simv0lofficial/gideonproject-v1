...
void CDrawing::Initialize()
{
	...
	if ( !m_init || oldw != w || oldh != h )
	{
		...
		Surface->SetFontGlyphSet(fonts.esp_main, "Tahoma", 12, 500, 0, 0, FONTFLAG_DROPSHADOW);
		Surface->SetFontGlyphSet(fonts.esp_name, "Verdana", 12, 600, 0, 0, FONTFLAG_DROPSHADOW);
		Surface->SetFontGlyphSet(fonts.esp_indicator, "Tahoma", 24, 700, 0, 0, FONTFLAG_DROPSHADOW);
		Surface->SetFontGlyphSet(fonts.esp_side, "Small Fonts", 8, 500, 0, 0, FONTFLAG_OUTLINE);
		Surface->SetFontGlyphSet(fonts.esp_icons, "Tahoma", 12, 500, 0, 0, FONTFLAG_DROPSHADOW);
