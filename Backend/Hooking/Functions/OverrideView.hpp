...
namespace Cheat
{
	namespace Hooked
	{
		void __stdcall OverrideViewHook( CViewSetup* pSetup )
		{
			...
			{
				if ( ( G::LocalPlayer->IsScoped() && c_config::get()->b["vis_nozoom"] ) || !G::LocalPlayer->IsScoped() )
				...
				if (shouldfd) {
					if (!G::IsInThirdPerson)
						...
						Vector end_point = Vector(G::LocalPlayer->GetAbsOrigin().x + 64.f, G::LocalPlayer->GetAbsOrigin().y + 64.f, G::LocalPlayer->GetAbsOrigin().z + 64.f);
