...
namespace ns_entity_list {
	...
	CBaseEntity* get_client_entity_from_handle(ULONG ent) {
		return EntityList->GetClientEntityFromHandle2(ent);
	}
}
namespace ns_utils {
	sol::table get_player_data(player_info_t p) {
		sol::table t = c_lua::get()->lua.create_table();
		t["name"] = std::string(p.name);
		t["guid"] = std::string(p.guid);
		t["userid"] = p.userid;
		return t;
	}
...
void c_lua::initialize() {
	...
	cheat["get_cheat_version"] = ns_cheat::get_cheat_version;
	cheat["get_game_name"] = ns_cheat::get_game_name;
	cheat["get_user_name"] = ns_cheat::get_user_name;
...
void c_lua::refresh_scripts() {
	...
	for (auto& entry : std::filesystem::directory_iterator( ("C:\\gideonproject\\lua"))) {
