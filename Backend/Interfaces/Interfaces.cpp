...
{
	CInterfaceManager* ClientManager			= new CInterfaceManager( Cheat::Settings->ClientDLL );
	CInterfaceManager* EngineManager			= new CInterfaceManager( xorstr_("engine.dll") );
	CInterfaceManager* VGUIMatSurfaceManager	= new CInterfaceManager( xorstr_("vguimatsurface.dll") );
	CInterfaceManager* VGUI2Manager				= new CInterfaceManager( xorstr_("vgui2.dll") );
	CInterfaceManager* VPhysicsManager			= new CInterfaceManager( xorstr_("vphysics.dll") );
	CInterfaceManager* VSTDLibManager			= new CInterfaceManager( xorstr_("vstdlib.dll") );
	CInterfaceManager* MaterialSYSManager		= new CInterfaceManager( xorstr_("materialsystem.dll") );
	CInterfaceManager* VStudioRender			= new CInterfaceManager( xorstr_("studiorender.dll") );
	CInterfaceManager* LocalizeManager			= new CInterfaceManager( xorstr_("localize.dll") );
	...
	Interfaces::MoveHelper = **reinterpret_cast<IMoveHelper***>(Cheat::Utilities->Memory_PatternScan(xorstr_("client_panorama.dll"), xorstr_("8B 0D ? ? ? ? 8B 45 ? 51 8B D4 89 02 8B 01")) + 2);
